...
# Script to update all pages with simple footers to comprehensive footer and floating buttons
# This will replace simple footers with the complete footer structure

echo "Starting comprehensive footer updates..."

# Function to check if file needs Link import
check_and_add_link_import() {
    local file="$1"
    if ! grep -q "import Link from" "$file"; then
        echo "Adding Link import to $file"
        # Add Link import after the first import or 'use client' directive
        sed -i '' "/^'use client';/a\\
...
update_file() {
    ...
    # Check and add Link import if needed
    check_and_add_link_import "$file"
    
    # Create a backup
    cp "$file" "${file}.backup"
    
    # Create temporary files
    local before_footer="/tmp/before_footer_$$.txt"
    local after_footer="/tmp/after_footer_$$.txt"
    
    # Split the file at the footer
    awk '/footer className="bg-gray-900 text-white py-12"/{exit} {print}' "$file" > "$before_footer"
    
    # Get everything after the footer (should be closing tags)
    awk '
    BEGIN { found=0; in_footer=0; brace_count=0 }
    ...
    found && in_footer {
        # Count braces to find the end of footer
        for(i=1; i<=length($0); i++) {
            char = substr($0, i, 1)
            if(char == "{") brace_count++
            if(char == "}") brace_count--
        }
        if(/\/footer>/ && brace_count <= 1) {
            in_footer=0
            next
        }
        if(brace_count <= 0) {
    ...
    found && !in_footer { print }
    ' "$file" > "$after_footer"
    
    # Create the comprehensive footer
    cat > /tmp/comprehensive_footer_$$.txt << 'EOF'
...
EOF
    ...
    # Combine: before footer + comprehensive footer + after footer
    cat "$before_footer" > "$file"
    cat "/tmp/comprehensive_footer_$$.txt" >> "$file"
    cat "$after_footer" >> "$file"
    
    # Clean up temp files
    rm -f "$before_footer" "$after_footer" "/tmp/comprehensive_footer_$$.txt"
    
    echo "Updated: $file"
}

# Test mode - update just one file first
if [ "$1" = "--test" ]; then
    test_file="/Users/mydigital/Documents/superpestcontrolnextjs/src/app/pest-control-airoli/page.tsx"
    if [ -f "$test_file" ]; then
        echo "Testing with file: $test_file"
        update_file "$test_file"
        echo "Test completed. Check the file and run without --test to update all files."
        exit 0
    else
...
# Get list of files with simple footers (py-12 class)
simple_footer_files=$(grep -l "py-12" /Users/mydigital/Documents/superpestcontrolnextjs/src/app/*/page.tsx)

echo "Found $(echo "$simple_footer_files" | wc -l) files with simple footers to update..."

# Process each file
for file in $simple_footer_files; do
    update_file "$file"
done

echo "Comprehensive footer updates completed!"
echo "Updated $(echo "$simple_footer_files" | wc -l) files"
