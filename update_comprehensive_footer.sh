...
# Create a temporary file with the comprehensive footer content
cat > /tmp/comprehensive_footer.txt << 'EOF'
...
for file in $simple_footer_files; do
    ...
    # Find the start of footer section and replace until the end of the component
    # This is a complex replacement, so we'll use a more targeted approach
    
    # Create a backup
    cp "$file" "${file}.backup"
    
    # Use awk to replace from footer to end of component
    awk '
    /footer className="bg-gray-900 text-white py-12"/ {
        # Read the comprehensive footer from temp file
        while ((getline line < "/tmp/comprehensive_footer.txt") > 0) {
            print line
        }
        close("/tmp/comprehensive_footer.txt")
        
        # Skip until we find the end of the current footer
        skip = 1
        brace_count = 0
        while (skip && (getline > 0)) {
            if (/<\/footer>/) {
                skip = 0
                break
            }
...
# Clean up temp file
rm -f /tmp/comprehensive_footer.txt
